...
This role allows you to deploy and start Odoo prod docker instances on your server.
Normally it should reuse the image tested and validated in test using [docker_odoo_test role](https://github.com/lefilament/ansible_role_odoo_test_docker), but you could also have only prod instance running.
...
- automatic bank statement retrieval nightly (using an homemade [Weboob](http://weboob.org) docker and cron jobs to run it every night for each bank account) - this part has been commented out for now since banking system is getting harder to webscrap and weboob is changing to woob
